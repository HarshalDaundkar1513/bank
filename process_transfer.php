...
$sqlCheckBalance = "SELECT SUM(amount) AS balance FROM transactions WHERE sender_name = ?";
$stmtCheckBalance = $conn->prepare($sqlCheckBalance);

if ($stmtCheckBalance === false) {
    die('Error: ' . $conn->error);
}

$stmtCheckBalance->bind_param("s", $sender_name);
$stmtCheckBalance->execute();
$stmtCheckBalance->bind_result($sender_balance);
$stmtCheckBalance->fetch();
$stmtCheckBalance->close();

// Perform the transfer if the sender's balance is sufficient
if ($sender_balance >= $amount) {
    ...
    echo "<script>alert('Transaction canceled. Insufficient balance.');</script>";
    echo "<script>window.location.href = 'index.php';</script>";
    exit();
}
?>
